...
(function (root, factory) {
			...
			if (index !== undefined) {
				...
				if (value === false) {
					...
					if (this.elt[index].innerHTML === this.elt[index].value) {
						this.elt[index].innerHTML = this.elt[index].value =
							value;
		...
		self.selected = function (value) {
			...
			if (value) {
				for (i = 0; i < inputChildren.length; i++) {
					if (inputChildren[i].value === value)
	...
	p5.Element.prototype.html = function () {
		if (arguments.length === 0) {
			...
			// Sanitize HTML before appending
			const sanitized = DOMPurify.sanitize(arguments[0]);
			this.elt.innerHTML += sanitized;
			return this;
		} else {
	...
	p5.Element.prototype._translate = function () {
		...
		if (this.elt.style.transform) {
			transform = this.elt.style.transform.replace(
				/translate3d\(.*\)/g,
				""
			);
			...
			if (aW !== AUTO || aH !== AUTO) {
				...
				if (this.elt instanceof HTMLCanvasElement) {
					...
					this.elt.setAttribute(
						"width",
						aW * this._pInst._pixelDensity
					);
					this.elt.setAttribute(
						"height",
						aH * this._pInst._pixelDensity
					);
					this.elt.setAttribute(
						"style",
	...
	p5.MediaElement = function (elt, pInst) {
		...
		Object.defineProperty(self, "src", {
			get: function () {
				...
				var ret =
					firstChildSrc === window.location.href
						? srcVal
						: firstChildSrc;
				return ret;
			},
			set: function (newValue) {
				...
				try {
					...
					console.error("Invalid media source URL:", e);
					return;
				}
	...
	p5.MediaElement.prototype.connect = function (obj) {
		...
		if (!this.audioSourceNode) {
			this.audioSourceNode = audioContext.createMediaElementSource(
				this.elt
			);
