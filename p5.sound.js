  ...
  */
		(function () {
			...
			if (
				...
				if (
					typeof AudioContext.prototype.createOscillator !==
					"function"
				) {
	...
	helpers = (function () {
		...
		p5.prototype.soundFormats = function () {
			...
			for (var i = 0; i < arguments.length; i++) {
				...
				if (
					["mp3", "wav", "ogg", "m4a", "aac"].indexOf(arguments[i]) >
					-1
				) {
				...
				if (["mp3", "wav", "ogg", "m4a", "aac"].indexOf(extTest) > -1) {
					if (p5.prototype.isFileSupported(extTest)) {
						...
						for (var i = 0; i < p5sound.extensions.length; i++) {
							...
							var supported =
								p5.prototype.isFileSupported(extension);
							if (supported) {
								pathCore = "";
		 ...
		 */
		p5.prototype._mathChain = function (
			o,
			math,
			thisChain,
			nextChain,
	...
	soundfile = (function () {
			...
			if (this.url !== undefined && this.url !== "") {
				...
				request.onload = function () {
					if (request.status === 200) {
						...
						ac.decodeAudioData(
							request.response, // success decoding buffer:
							function (buff) {
								self.buffer = buff;
								self.panner.inputChannels(
			...
			for (var index = 0; index < peaksArray.length; index++) {
				...
				for (var i = 0; i < 10; i++) {
					...
					if (startPeak && endPeak) {
						...
						var foundInterval = intervalCounts.some(function (
							intervalCount
						) {
		...
		function getPeaksAtTopTempo(peaksObj, tempo, sampleRate, bpmVariance) {
			...
			peaksAtTopTempo = peaksAtTopTempo.filter(function (
				peakTime,
				index,
				arr
			) {
		...
		p5.Amplitude.prototype._audioProcess = function (event) {
			for (
				...
				this.stereoVol[channel] = Math.max(
					rms,
					this.stereoVol[channel] * this.smoothing
				);
				this.stereoAvg[channel] = Math.max(
					average,
					this.stereoVol[channel] * this.smoothing
				);
			...
			var self = this;
			var volSum = this.stereoVol.reduce(function (
				previousValue,
				currentValue,
				index
			) {
				self.stereoVolNorm[index - 1] = Math.max(
					Math.min(self.stereoVol[index - 1] / self.volMax, 1),
					0
				);
				self.stereoVolNorm[index] = Math.max(
					Math.min(self.stereoVol[index] / self.volMax, 1),
					0
				);
	...
	fft = (function () {
		...
		p5.FFT.prototype.getFreq = function (freq1, freq2) {
			console.log(
				"getFreq() is deprecated. Please use getEnergy() instead."
			);
     ...
     */
		...
		p5.FFT.prototype.getOctaveBands = function (N, fCtr0) {
			...
			while (lastFrequencyBand.hi < nyquist) {
				...
				newFrequencyBand.ctr =
					lastFrequencyBand.ctr * Math.pow(2, 1 / N);
				newFrequencyBand.hi =
					newFrequencyBand.ctr * Math.pow(2, 1 / (2 * N));
		...
		var freqToFloat = function (fft) {
			if (fft.freqDomain instanceof Float32Array === false) {
				fft.freqDomain = new Float32Array(
					fft.analyser.frequencyBinCount
				);
			}
		};
		...
		var timeToFloat = function (fft) {
			if (fft.timeDomain instanceof Float32Array === false) {
				fft.timeDomain = new Float32Array(
					fft.analyser.frequencyBinCount
				);
			}
		};
	...
	Tone_core_Tone = (function () {
		...
		Tone.prototype.toString = function () {
			for (var className in Tone) {
				...
				if (
					this.isFunction(Tone[className]) &&
					isLetter &&
					sameConstructor
				) {
					return className;
	...
	Tone_type_Time = (function (Tone) {
		...
		Tone.Time.prototype.toNotation = function () {
			...
			var retTripletNotation = this._toNotationHelper(
				time,
				testTripletNotations
			);
			if (
				retTripletNotation.split("+").length <
	...
	Tone_type_Frequency = (function (Tone) {
		...
		Tone.Frequency.prototype._ticksToUnits = function (ticks) {
			return (
				1 /
				((ticks * 60) / (Tone.Transport.bpm.value * Tone.Transport.PPQ))
			);
		};
	...
	Tone_core_Context = (function (Tone) {
		...
		Tone.Context.prototype._createWorker = function () {
			...
			var blob = new Blob([
				"var timeoutTime = " +
					(this._updateInterval * 1000).toFixed(1) +
					";" +
				...
				function () {
					...
					if (this.isNumber(this._lastUpdate)) {
						...
						this._computedUpdateInterval = Math.max(
							diff,
							this._computedUpdateInterval * 0.97
						);
					}
	...
	oscillator = (function () {
		 ...
		 *                                     with scaled output
		 */
		p5.Oscillator.prototype.scale = function (
			inMin,
			inMax,
			outMin,
			outMax
	...
	Tone_signal_TimelineSignal = (function (Tone) {
		...
		Tone.TimelineSignal.prototype.connect =
			Tone.SignalBase.prototype.connect;
	...
	env = (function () {
		...
		p5.Env.prototype.play = function (unit, secondsFromNow, susTime) {
			...
			this.triggerRelease(
				unit,
				tFromNow + this.aTime + this.dTime + susTime
			);
		};
		/**
	...
	audioin = (function () {
			...
			window.navigator.mediaDevices
				...
				.catch(function (err) {
					if (errorCallback) errorCallback(err);
						...
						alert(
							"Unable to access microphone. Please ensure microphone permissions are granted and try again."
						);
					}
				});
		};
			...
			return new Promise(function (resolve, reject) {
				window.navigator.mediaDevices
					...
					.then(function (devices) {
						p5sound.inputSources = devices.filter(function (
							device
						) {
		...
		p5.AudioIn.prototype.setSource = function (num) {
			if (
				p5sound.inputSources.length > 0 &&
				num < p5sound.inputSources.length
			) {
				// set the current source
				this.currentSource = num;
				console.log(
					"set source to ",
					p5sound.inputSources[this.currentSource]
				);
			} else {
	...
	Tone_signal_Expr = (function (Tone) {
		...
		Tone.Expr = function () {
			...
			try {
				...
				this._disposeNodes();
				throw new Error(
					"Tone.Expr: Could evaluate expression: " + expr
				);
			}
			this.output = result;
		};
		...
		Tone_core_Tone,
		Tone_signal_Add,
		Tone_signal_Subtract,
		Tone_signal_Multiply,
		Tone_signal_GreaterThan,
		Tone_signal_GreaterThanZero,
		Tone_signal_Abs,
		Tone_signal_Negate,
		Tone_signal_Modulo,
		Tone_signal_Pow
	...
	src_eqFilter = (function () {
		...
		EQFilter.prototype.amp = function () {
			console.warn(
				"`amp()` is not available for p5.EQ bands. Use `.gain()`"
			);
		};
	...
	panner3d = (function () {
		 ...
		 */
		p5.Panner3D.prototype.setFalloff = function (
			maxDistance,
			rolloffFactor
		) {
	...
	listener3d = (function () {
		...
		p5.Listener3D.prototype.orientUp = function (
			xValU,
			yValU,
			zValU,
			time
	...
	delay = (function () {
		...
		p5.Delay = function () {
			...
			this._rightFilter.biquad.gain.setValueAtTime(
				1,
				this.ac.currentTime
			);
			// default routing
		 ...
		 *                               delay.
		 */
		p5.Delay.prototype.process = function (
			src,
			_delayTime,
			_feedback,
			_filter
	...
	reverb = (function () {
		 ...
		 */
		p5.Reverb.prototype.process = function (
			src,
			seconds,
			decayRate,
			reverse
		 ...
		 *  </code></div>
		 */
		p5.prototype.createConvolver = function (
			path,
			callback,
			errorCallback
		) {
	...
	Tone_core_Clock = (function (Tone) {
		...
		Tone.Clock.prototype._loop = function () {
			...
			var loopInterval =
				now + lookAhead + updateInterval + lagCompensation;
			...
			if (elapsedTime - this.tatumTime <= -0.02) {
				...
				this.syncedParts.forEach(function (thisPart) {
					...
					thisPart.phrases.forEach(function (thisPhrase) {
						...
						if (
							phraseArray[bNum] !== 0 &&
							(self.metroTicks < phraseArray.length ||
								!thisPhrase.looping)
						) {
							thisPhrase.callback(
								secondsFromNow,
	...
	soundloop = (function () {
		...
		p5.SoundLoop = function (callback, interval) {
			...
			this.musicalTimeMode =
				typeof this._interval === "number" ? false : true;
			...
			this.clock = new Clock({
				callback: function (time) {
					 ...
					 * This ususally occurs for a few milliseconds when the page
					 * is not fully loaded
					 *
		...
		Object.defineProperty(p5.SoundLoop.prototype, "interval", {
			...
			set: function (interval) {
				this.musicalTimeMode =
					typeof interval === "Number" ? false : true;
	...
	soundRecorder = (function () {
		...
		p5.SoundRecorder = function () {
			...
			this._jsNode = ac.createScriptProcessor(
				this.bufferSize,
				this._channels,
				2
			);
		 ...
		 */
		p5.SoundRecorder.prototype.record = function (
			sFile,
			duration,
			callback
		) {
		...
		p5.SoundRecorder.prototype._audioprocess = function (event) {
			if (this.recording === false) {
				...
				if (
					this.sampleLimit &&
					this.recordedSamples >= this.sampleLimit
				) {
					this.stop();
				} else {
					// get channel data
	...
	peakdetect = (function () {
		...
		p5.PeakDetect.prototype.update = function (fftObject) {
			var nrg = (this.energy =
				fftObject.getEnergy(this.f1, this.f2) / 255);
			if (
				nrg > this.cutoff &&
				nrg > this.threshold &&
				nrg - this.penergy > 0
	...
	audioVoice = (function () {
		...
		p5.AudioVoice.prototype._setNote = function (note) {
			...
			var octave =
				typeof Number(note.slice(-1)) === "number" ? note.slice(-1) : 0;
			value += 12 * octave;
			value =
				note[1] === "#"
					? value + 1
	...
	monosynth = (function () {
		 ...
		 *  @param {Number} [releaseTime]   Time in seconds from now (defaults to 0)
		 */
		p5.MonoSynth.prototype.setADSR = function (
			attack,
			decay,
			sustain,
			release
	...
	polysynth = (function () {
		...
		p5.PolySynth = function (audioVoice, polyValue) {
			 ...
			 */
			this.AudioVoice =
				audioVoice === undefined ? p5.MonoSynth : audioVoice;
			/**
		...
		p5.PolySynth.prototype.noteADSR = function (
			...
			this.audiovoices[this.notes[note].getValueAtTime(t)].setADSR(
				a,
				d,
				s,
				r
		...
		p5.PolySynth.prototype.noteAttack = function (
			...
			var note =
				typeof _note === "string"
					? this.AudioVoice.prototype._setNote(_note)
					: typeof _note === "number"
					? _note
					: 440;
			...
			if (
				...
				this.notes[note].getValueAtTime(t) !== null
			) {
			...
			var previousVal =
				this._voicesInUse._searchBefore(t) === null
					? 0
					: this._voicesInUse._searchBefore(t).value;
		...
		p5.PolySynth.prototype.noteRelease = function (_note, secondsFromNow) {
			...
			var note =
				typeof _note === "string"
					...
					: typeof _note === "number"
					? _note
					: this.audiovoices[this._newest].oscillator.freq().value;
			var now = p5sound.audiocontext.currentTime;
			var tFromNow = secondsFromNow || 0;
			var t = now + tFromNow;
			if (this.notes[note].getValueAtTime(t) === null) {
				console.warn(
					"Cannot release a note that is not already playing"
				);
			} else {
				//Find the scheduled change in this._voicesInUse that will be previous to this new note
				...
				this._updateAfter(t, -1);
				this.audiovoices[
					this.notes[note].getValueAtTime(t)
				].triggerRelease(tFromNow);
				this.notes[note].setValueAtTime(null, t);
				this._newest =
					this._newest === 0
						? 0
						: (this._newest - 1) % (this.polyValue - 1);
			}
		};
		/**
		 *  Connect to a p5.sound / Web Audio object.
